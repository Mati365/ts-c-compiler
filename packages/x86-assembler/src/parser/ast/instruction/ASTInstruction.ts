...
import { reduceTextToBitset } from '@ts-cc/core';
import { NodeLocation } from '@ts-cc/grammar';
import { TokensIterator } from '@ts-cc/grammar';
import { NumberToken, TokenType, TokenKind, Token } from '@ts-cc/lexer';
...
export class ASTInstruction extends KindASTAsmNode(ASTNodeKind.INSTRUCTION) {
  ...
  get segMemArgs() {
    return <ASTInstructionMemSegmentedArg[]>(
      this.typedArgs[InstructionArgType.SEGMENTED_MEMORY]
    );
  }
  ...
  clone(): ASTInstruction {
    ...
    const cloned = new ASTInstruction(opcode, null, prefixes, loc);
  ...
  findRMArg(): ASTInstructionMemPtrArg {
    return <ASTInstructionMemPtrArg>(
      R.find<ASTInstructionArg>(arg => arg.schema?.rm, this.args)
    );
  }
  ...
  parseInstructionArgsTokens(
    ...
    argsTokensIterator.iterate((token: Token) => {
      ...
      if (
        result.byteSize &&
        (result.type === InstructionArgType.MEMORY ||
          result.type === InstructionArgType.REGISTER)
      ) {
      ...
      const sizeMismatch =
        ...
        (prevArg.type === InstructionArgType.REGISTER ||
          prevArg.type === InstructionArgType.NUMBER) &&
  ...
  static parse(token: Token, parser: ASTAsmParser): ASTInstruction {
    ...
    if (
      ...
      (argsTokens[1].type === TokenType.KEYWORD ||
        argsTokens[1].type === TokenType.NUMBER)
    ) {
