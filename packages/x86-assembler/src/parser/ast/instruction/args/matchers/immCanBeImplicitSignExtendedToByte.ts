import { toUnsignedNumber, signExtend } from '../../../../../utils';
...
export function immCanBeImplicitSignExtendedToByte(
  ...
  const originalValue = toUnsignedNumber(numArg.val, targetSize);
  const extended = signExtend(
    originalValue,
    srcByteSize,
    <X86BitsMode>numArg.signedByteSize,
  );

  return toUnsignedNumber(extended, targetSize) === originalValue;
}
