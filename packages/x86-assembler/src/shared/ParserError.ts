...
export const ERROR_TRANSLATIONS: Record<ParserErrorCode, string> = {
  ...
  [ParserErrorCode.EXCEEDING_CASTED_NUMBER_SIZE]:
    'Provided value "%{value}" is exceeding casted arg size (provided %{size} bytes but max is %{maxSize} bytes)!',

  // other args
  [ParserErrorCode.INCORRECT_ARG_QUOTE_TEXT_LENGTH]:
    'Incorrect text "%{text}" length! It must be less than %{maxSize} bytes!',
  ...
  [ParserErrorCode.INCORRECT_MODRM]:
    'Error during "%{phrase}" ModRM instruction byte parsing!',
  [ParserErrorCode.DISPLACEMENT_EXCEEDING_BYTE_SIZE]:
    'Displacement of "%{address}" exceedes arg byte size (%{byteSize} bytes but should be <= %{maxSize} bytes)!',
  [ParserErrorCode.SCALE_INDEX_IS_UNSUPPORTED_IN_MODE]:
    'Scale index is unsupported in this mode!',

  // segmented mem
  [ParserErrorCode.INCORRECT_SEGMENTED_MEM_FORMAT]:
    'Incorrect segmented memory format "%{address}"!',
  [ParserErrorCode.INCORRECT_SEGMENTED_MEM_ARGS_COUNT]:
    'Incorrect segmented memory address args count %{count}!',

  [ParserErrorCode.INCORRECT_SEGMENT_MEM_ARG_SIZE]:
    'Incorrect address segment size, provided %{size} bytes but required is 2 bytes!',
  [ParserErrorCode.INCORRECT_OFFSET_MEM_ARG_SIZE]:
    'Incorrect address offset size, provided %{size} bytes but required is <= 4 bytes!',
  [ParserErrorCode.OFFSET_MEM_ARG_SIZE_EXCEEDING_SIZE]:
    'Incorrect address offset size, provided %{size} bytes but should be <= %{maxSize} bytes!',

  // prefixes
  [ParserErrorCode.INCORRECT_SREG_OVERRIDE]:
    'Incorrect segment register override "%{sreg}"!',
  ...
  [ParserErrorCode.UNABLE_PARSE_REPEATED_INSTRUCTION]:
    'Unable to parse repeated instruction "%{expression}"!',
  ...
  [ParserErrorCode.UNKNOWN_COMPILER_INSTRUCTION]:
    'Unknown compile instruction "%{instruction}"!',
  ...
  [ParserErrorCode.MISSING_RM_BYTE_DEF]:
    'Missing RM byte arg definition but in binary schema is present!',
  ...
  [ParserErrorCode.INCORRECT_EQU_ARGS_COUNT]:
    'Incorrect equ args count, provided %{count} but required is 1!',
