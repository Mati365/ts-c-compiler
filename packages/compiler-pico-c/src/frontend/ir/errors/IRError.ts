...
export const C_IR_ERROR_TRANSLATIONS: Record<IRErrorCode, string> = {
  ...
  [IRErrorCode.VARIABLE_MUST_BE_PRIMITIVE]: fixme('Variable %{name} must be primitive!'),
  [IRErrorCode.INCORRECT_INITIALIZER_BLOCK]: fixme('Incorrect initializer block!'),
  [IRErrorCode.UNABLE_TO_COMPILE_EXPRESSION]: fixme('Unable to compile expression!'),
  [IRErrorCode.UNRESOLVED_IDENTIFIER]: fixme('Unresolved expression IR output variable!'),
  [IRErrorCode.ACCESS_ARRAY_INDEX_TO_NON_ARRAY]: fixme('Trying to access non array type via index!'),
  [IRErrorCode.ACCESS_STRUCT_ATTR_IN_NON_STRUCT]: fixme('Trying to access struct field on non struct type!'),
 ...
 * @export
 * @class IRError
 * @extends {CompilerError<IRErrorCode, null>}
 */
