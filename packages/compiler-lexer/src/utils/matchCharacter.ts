...
export function matchQuote(c: string): TokenKind {
  switch (c) {
    case "'":
      return TokenKind.SINGLE_QUOTE;
    case '"':
      return TokenKind.DOUBLE_QUOTE;
...
export function matchBracket(c: string): TokenKind {
  switch (c) {
    case '(':
    case ')':
      return TokenKind.PARENTHES_BRACKET;
    case '{':
    case '}':
      return TokenKind.CURLY_BRACKET;
    case '[':
    case ']':
...
export function toStringBracketToken(c: string, flip: boolean = false): string {
  ...
  switch (c) {
    case TokenKind.PARENTHES_BRACKET:
      character = '(';
      break;
    case TokenKind.CURLY_BRACKET:
      character = '{';
      break;
    case TokenKind.SQUARE_BRACKET:
      character = '[';
      break;
  ...
  return flip ? flipBracket(character) : character;
}
