...
import { X86Flags, X86Interrupt, X86InterruptType } from './parts';
  ...
  static initBaseOpcodes(cpu: X86CPU): void {
    ...
    Object.assign(opcodes, {
      ...
      /** CALL 16bit/32bit dis  */ 0xe8: () => {
        stack.push(toUnsignedNumber(registers.ip + 0x2, 0x2));
      ...
      /** XLAT */ 0xd7: () => {
        registers.al = memIO.read[0x1](
          (registers[<string>cpu.segmentReg] << 4) +
            toUnsignedNumber(registers.bx + registers.al, 0x2),
        );
      },
