...
(() => {
  ...
  const mappedInstructions = [...opcodesTables].map(
    (table) => {
      const rows = table.querySelectorAll('tr:not(:first-child)');

      return [...rows].map(
        (row) => [...row.querySelectorAll('tt')]
          .map(
            (col) => col.innerHTML.split('&nbsp;').filter((str) => str.length),
          ),
      );
    },
  );
  ...
  const groupedInstructions = mappedInstructions
    .reduce(
      (acc, list) => {
        list
          .sort(
            (a, b) => pickBinaryOpcode(a) - pickBinaryOpcode(b),
          )
  ...
  const prettyPrintArray = (json) => {
    ...
    const output = JSON.stringify(json, (k, v) => {
