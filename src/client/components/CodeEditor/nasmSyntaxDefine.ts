...
export function nasmSyntaxDefine(): CodeMirror.Mode<any> {
  const conventionalInstructions = words(
  ...
  const specialInstructions = words('db dw dd dq dt do dy dz resb resw resd resq rest reso resy resz');
  const sections = words('section segment .data .text .bss');
